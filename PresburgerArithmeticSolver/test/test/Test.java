...
public class Test {
	...
	public void testCooper1() {
		...
		assertEquals("Ex.~Ey.(3x+1>=10&7x-6<=7+y)/~2|x", root.toString());
		root = new ToLessThan(root).go();
		assertEquals("Ex.~Ey.(SUM[10]<SUM[3x, 1, 1]&SUM[7x, -6]<SUM[7, y, 1])/~2|x", root.toString());
	...
	public void testCooper2() {
		...
		root = new ToLessThan(root).go();
		assertEquals("~Ex.20 + x < 0 + 1 & (~Ey.3y + x < 10 + 1 & 20 < y - x + 1)", root.toString());
		// isolate y under Ey.
		root = new VariableIsolater(root).go();
		assertEquals("~Ex.20 + x < 0 + 1 & (~Ey.3y < 10 + 1 - x & x - 1 + 20 < y)", root.toString());
		// compact comparator children under Ey.
		root = new Compacter(root).go();
		assertEquals("~Ex.x + 20 < 1 & (~Ey.3y < -x + 11 & x + 19 < y)", root.toString());
		// gcd on y factor under Ey.
		assertEquals("~Ex.0 < 20 + x & (~Ey.3y < -x + 11 & 3x + 57 < 3y)", root.toString());
		// replace 3y with y1 (and add divisor constraint)
		assertEquals("~Ex.0 < 20 + x & (~Ey1.y1 < 11 - x & 3x + 57 < y1 & 3|y1)", root.toString());
